...
def findStocks():
    ...
    random.shuffle(stocks,random.random)
...
class valuations:
    ...
    def start(self):
        try:
            time.sleep(4)
            self.url = r"http://financials.morningstar.com/valuation/price-ratio.html?t={0}&region=usa&culture=en-US".format(self.stock.upper())
            ...
            try:
                ...
                with setTimeout(seconds = wait_seconds,error_message = error_message):
                    ...
                    if soup.findAll("table"):
                        ...
                        log("Something failed on Valuations. Raising the number of Loads ... ")
                        loading_page += 1
    ...
    def getData(self):
        ...
        for df in dfs:
            try:
                ...
                pass
        five_years_avg_column = self.stock.upper() + " 5Y Avg*"
        five_years_avg = coke_df[five_years_avg_column].tolist()
        one_year = coke_df[self.stock.upper()].tolist()
        self.fiver_years_avg_val = five_years_avg[0]
        self.one_year_avg_val = one_year[0]  
        log("Five years : {0} . One year : {1}".format(self.fiver_years_avg_val.encode('utf-8'),self.one_year_avg_val.encode('utf-8')))
